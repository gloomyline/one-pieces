<template>
    <div class="partner">
        ...
        <el-table :data="tableData" stripe border style="width: 100%;">
            ...
            <el-table-column property="image" label="图片" align="center" min-width="80">
                <template slot-scope="scope">
                    ...
                    <el-button v-popover:popover4 v-if="scope.row.image" style="padding: 0"><img :src="scope.row.image" width="60" height="21"/></el-button>
        ...
        <div>
            <el-dialog  @close="handleClose" :visible.sync="dialogFormVisible" title="合作伙伴">
                <el-form :model="form" ref="form" label-width="120px" label-position="left">
                    <el-form-item label="名称" prop="name" :rules="[{ required: true, message: '合作伙伴名称' }]">
                    ...
                    <el-form-item label="图片">
                        <el-upload
                            ...
                            <div slot="tip" class="el-upload__tip">只能上传jpg/png文件，且不超过2MB,注：合作伙伴图片最佳展示尺寸为200px*70px</div>
                        </el-upload>
                    ...
                    <el-form-item label="URL链接地址" prop="link">
                        <el-input v-model="form.link" placeholder="例：http://www.onepieces.cn" :maxlength="50" style="width:210px;"></el-input>
                    </el-form-item>
  ...
  export default {
    ...
    methods: {
      getData() {
        ...
        this.$http.get(`${apiBase}partner`, { params })
      ...
      handleDel(id) {
        this.$confirm('确定删除该合作伙伴信息?', '提示', {
          ...
          this.$http.post(`${apiBase}partner-del`, { partner_id: id })
            .then(response => response.json())
            .then((json) => {
              if (json.status !== 'SUCCESS') {
      ...
      handleSubmit() {
        ...
        this.$http.post(`${apiBase}partner-add`, params)
          .then(response => response.json())
          .then((json) => {
            if (json.status !== 'SUCCESS') {
              return Promise.reject(json.error_message);
      ...
      handleSubmitUpdate() {
        ...
        this.$http.put(`${apiBase}partner-update`, params)
          .then(response => response.json())
          .then((json) => {
            if (json.status !== 'SUCCESS') {
              return Promise.reject(json.error_message);
...
<style>
    .partner .image-uploader .el-upload {
    ...
    .partner .image-uploader .el-upload:hover {
        border-color: #409EFF;
    }
    .partner .image-uploader-icon {
        font-size: 28px;
        color: #8c939d;
        width: 200px;
        height: 70px;
