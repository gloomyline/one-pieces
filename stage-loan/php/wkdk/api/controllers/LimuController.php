        ...
        if ($code == LimuService::API_SUCCESS_CODE) {
            switch ($bizType) {
                ...
                case LimuService::API_BIZ_EBANK:
                    ...
                    break;
                //公积金查询回调
                case LimuService::API_BIZ_HOUSE_FUND:
                    $jsonResult = LimuService::commonResultGet($token, $bizType);
                    $jsonObj = json_decode($jsonResult);
                    if ($jsonObj->code == LimuService::API_SUCCESS_CODE) {
                        $result = UserService::setUserLimuAuth($token, $jsonObj->data, UserLimuModel::TYPE_HOUSE_FUND);
                    } else {
                        Yii::error("\r\n" . '立木回调失败（公积金查询）:' . "\r\n" . 'code:' . $jsonObj->code . "\r\n" . $jsonObj->msg, 'limu');

                        $im = Yii::$app->companyim;
                        $im->sendNotice('sys_error', $im::TECH_DEPT_CHAT_ID, '立木回调失败（公积金查询）:' . "\r\n" . 'code:' . $jsonObj->code . "\r\n" . $jsonObj->msg);
                    }
                    break;
                // 社保查询回调
                case LimuService::API_BIZ_SOCIAL_SECURITY:
                    $jsonResult = LimuService::commonResultGet($token, $bizType);
                    $jsonObj = json_decode($jsonResult);
                    if ($jsonObj->code == LimuService::API_SUCCESS_CODE) {
                        $result = UserService::setUserLimuAuth($token, $jsonObj->data, UserLimuModel::TYPE_SOCIAL_SECURITY);
                    } else {
                        Yii::error("\r\n" . '立木回调失败（社保查询）:' . "\r\n" . 'code:' . $jsonObj->code . "\r\n" . $jsonObj->msg, 'limu');

                        $im = Yii::$app->companyim;
                        $im->sendNotice('sys_error', $im::TECH_DEPT_CHAT_ID, '立木回调失败（社保查询）:' . "\r\n" . 'code:' . $jsonObj->code . "\r\n" . $jsonObj->msg);
                    }
                    break;
                // 央行征信查询回调
                case LimuService::API_BIZ_CREDIT:
                    $jsonResult = LimuService::commonResultGet($token, $bizType);
                    $jsonObj = json_decode($jsonResult);
                    if ($jsonObj->code == LimuService::API_SUCCESS_CODE) {
                        $result = UserService::setUserLimuAuth($token, $jsonObj->data, UserLimuModel::TYPE_CREDIT);
                    } else {
                        Yii::error("\r\n" . '立木回调失败（央行征信查询）:' . "\r\n" . 'code:' . $jsonObj->code . "\r\n" . $jsonObj->msg, 'limu');

                        $im = Yii::$app->companyim;
                        $im->sendNotice('sys_error', $im::TECH_DEPT_CHAT_ID, '立木回调失败（央行征信查询）:' . "\r\n" . 'code:' . $jsonObj->code . "\r\n" . $jsonObj->msg);
                    }
                    break;
                default:
                    break;
