...
class StatisticController extends BackendController
{
    ...
    public function actionIndex()
    {
        ...
        $todayRepayment = BudgetPlanModel::statRepaymentByDate($today);  // 今日还款总额 -- 等于还款计划中还款日为当天的统计

        $todayRefuse = LoanModel::todayRefuse(); // 今日拒绝次数--初审失败&&复审失败&&商家确认失败
        // 本期还款日
        $repayingDate = SiteService::getRecentRepayingDate($today);
        // 上个还款日
        $lastRepayingDate = date('Y-m-d',strtotime("$repayingDate - 1 month"));

        $todayOverdue = BudgetPlanModel::statOverdueByDate($lastRepayingDate); // 本期逾期未还总额

        // 商户总览 商户总数 放款总额 还款总额
        $totalShopCount = ShopModel::countShopStatePass(); // 商户总数
        $shopArrivalAmount = LoanModel::shopArrivalAmount(); // 商户放款总额
        $shopRepayAmount = LoanModel::shopRepaymentAmount(); // 商户还款总额

        // 商品总览 上 下 全部
        $totalProductCount = ShopProductModel::countByCondition(); // 全部商品
        $onSaleProductCount = ShopProductModel::countByCondition(['on_sale' => ShopProductModel::IS_ON_SALE]);


       $todayPlanRepayment = BudgetPlanModel::todayPlanRepayment($repayingDate);  // 本期待还总额（待还+逾期）

        $quotaApplyStateAuditingCount = QuotaApplyModel::getQuotaApplyStateAuditingCount(); // 额度待审核(用户)

        $shopQuotaApplyStateAuditingCount = ShopQuotaApplyModel::getShopQuotaApplyStateAuditingCount(); // 商户额度待审
        ...
        $quotaCount = LoanModel::getCountByState([LoanModel::STATE_REVIEW_SUCCESS, LoanModel::STATE_CONFIRM_SUCCESS]); // 待放款

        $overdueCount = LoanModel::getCountByState(LoanModel::STATE_OVERDUE); // 逾期未还

        // 逾期贷款记录最新逾期记录
        $overdueList = [];
        $lastOverdueList = BudgetPlanModel::getLastOverdue();
        if ($lastOverdueList) {
            foreach ($lastOverdueList as $list) {
                $overdueList[] = [
                    'id' => $list->id,
                    'encoding' => $list->loan->encoding ?? '', // 借款编号
                    'created_at' => $list->loan->created_at ?? '', // 借款日期
                    'period' => $list->loan->period ?? 0, // 借款期数
                    'term' => $list->term ?? 0, // 当前逾期期数
                    'quota' => $list->loan->quota ?? 0, // 本金
                    'monthly' => $list->monthly ?? 0, // 本期应还
        ...
        $weekLoan = LoanModel::dailyLoan(); // 一周借款
        $weekRepayment = BudgetPlanModel::dailyRepayment(); // 一周还款
        $weekApply = LoanModel::dailyApply(); // 一周申请次数
        $weekCheckRefuse = LoanModel::dailyCheckRefuse(); // 初审拒绝
        $weekReviewRefuse = LoanModel::dailyReviewRefuse(); // 复审拒绝
        $weekConfirmRefuse = LoanModel::dailyConfirmRefuse(); // 商家确认失败

        $result = [];
        $result = self::dataFormatter($week, $weekLoan, $weekRepayment, $weekApply, $weekCheckRefuse, $weekReviewRefuse, $weekConfirmRefuse);
        $data = [
            ...
            // 商户总览
            'total_shop_count' => $totalShopCount ?? 0, // 商户总数
            'shop_arrival_amount' => $shopArrivalAmount ?? 0, // 放款总额
            'shop_repayment_amount' => $shopRepayAmount ?? 0, // 还款总额
            // 商品总览
            'total_product_count' => $totalProductCount, // 全部商品
            'on_sale_product_count' => $onSaleProductCount, // 上架商品
            'auditingCount' => $auditingCount, // 待初审
            'reviewingCount' => $reviewingCount, // 待复审
            'quota_apply_state_auditing_count' => $quotaApplyStateAuditingCount, // 额度待审核用户
            'shop_quota_apply_state_auditing_count' => $shopQuotaApplyStateAuditingCount, // 额度待审核商户
    ...
    public function actionAccumulateStatistic()
    {
        ...
        $repayment = BudgetPlanModel::repaymentTotal(); // 还款总额

        $refuse = LoanModel::refuseTotal(); // 拒绝次数

        $overdue = BudgetPlanModel::overdueTotal(); // 逾期未还总额

        $planRepayment = BudgetPlanModel::planRepayment(); // 待还总额

        $months = self::getMonth();

        $monthLoan = LoanModel::monthLoan(); // 半年借款

        $monthRepayment = BudgetPlanModel::monthRepayment(); // 半年还款--后续可能会修改
        ...
        $monthConfirmRefuse = LoanModel::monthConfirmRefuse(); // 商户确认失败

        $result = [];
        $result = self::dataFormatter($months, $monthLoan, $monthRepayment, $monthApply, $monthCheckRefuse, $monthReviewRefuse, $monthConfirmRefuse);
     ...
     * @param array $confirmRefuse 商家确认失败
     * @return array 返回格式化的数组
     */
    private static function dataFormatter($length = [], $loan = [], $repayment = [], $apply = [], $checkRefuse = [], $reviewRefuse = [], $confirmRefuse = [])
    {
        $loans = [];
        $repayments = [];
        $applys = [];
