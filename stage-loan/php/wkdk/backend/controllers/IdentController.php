...
class IdentController extends BackendController
{
    ...
    public function actionLimuInfo()
    {
        ...
        // 养老明细分页
        $pensionOffset = $request->get('pension_offset', 0);
        $pensionLimit = $request->get('pension_limit', 10);
        // 医疗明细信息分页
        $medicareOffset = $request->get('medicare_offset', 0);
        $medicareLimit = $request->get('medicare_limit', 10);
        // 失业明细信息分页
        $jobSecurityOffset = $request->get('jobSecurity_offset', 0);
        $jobSecurityLimit = $request->get('jobSecurity_limit', 10);
        // 工伤明细信息分页
        $employmentInjuryOffset = $request->get('employmentInjury_offset', 0);
        $employmentInjuryLimit = $request->get('employmentInjury_limit', 10);
        // 生育明细信息分页
        $maternityOffset = $request->get('maternity_offset', 0);
        $maternityLimit = $request->get('maternity_limit', 10);

        $detail = UserLimuModel::getUserLimuById($id); //查询明细
        ...
        // 社保认证
        if ($detail->platform_type == UserLimuModel::TYPE_SOCIAL_SECURITY) {
            // 养老明细信息
            if (isset($result['pensionDetails']) && $result['pensionDetails']) {
                $result['pension_has_more'] = true; // 标识是否存在更多养老明细信息 true-存在 false-不存在
                if (!isset($result['pensionDetails'][$pensionOffset + $pensionLimit])) {
                    $result['pension_has_more'] = false;
                }
                $result['pensionDetails'] = array_slice($result['pensionDetails'], $pensionOffset, $pensionLimit);
            }
            // 医疗明细信息
            if (isset($result['medicareDetails']) && $result['medicareDetails']) {
                $result['medicare_has_more'] = true; // 标识是否存在更多医疗明细信息 true-存在 false-不存在
                if (!isset($result['medicareDetails'][$medicareOffset + $medicareLimit])) {
                    $result['medicare_has_more'] = false;
                }
                $result['medicareDetails'] = array_slice($result['medicareDetails'], $medicareOffset, $medicareLimit);
            }
            // 失业明细信息
            if (isset($result['jobSecurityDetails']) && $result['jobSecurityDetails']) {
                $result['jobSecurity_has_more'] = true; // 标识是否存在更多失业明细信息 true-存在 false-不存在
                if (!isset($result['jobSecurityDetails'][$jobSecurityOffset + $jobSecurityLimit])) {
                    $result['jobSecurity_has_more'] = false;
                }
                $result['jobSecurityDetails'] = array_slice($result['jobSecurityDetails'], $jobSecurityOffset, $jobSecurityLimit);
            }
            // 工伤明细信息
            if (isset($result['employmentInjuryDetails']) && $result['employmentInjuryDetails']) {
                $result['employmentInjury_has_more'] = true; // 标识是否存在更多工伤明细信息 true-存在 false-不存在
                if (!isset($result['employmentInjuryDetails'][$employmentInjuryOffset + $employmentInjuryLimit])) {
                    $result['employmentInjury_has_more'] = false;
                }
                $result['employmentInjuryDetails'] = array_slice($result['employmentInjuryDetails'], $employmentInjuryOffset, $employmentInjuryLimit);
            }
            // 生育明细信息
            if (isset($result['maternityDetails']) && $result['maternityDetails']) {
                $result['maternity_has_more'] = true; // 标识是否存在更多生育明细信息 true-存在 false-不存在
                if (!isset($result['maternityDetails'][$maternityOffset + $maternityLimit])) {
                    $result['maternity_has_more'] = false;
                }
                $result['maternityDetails'] = array_slice($result['maternityDetails'], $maternityOffset, $maternityLimit);
            }
        }
        return Json::encode([
            'status' => self::STATUS_SUCCESS,
     ...
     * @return string
     */
    public function actionHouseFundAuth()
    {
        $data = [];
        $request = Yii::$app->request;
        $offset = $request->get('offset', 0);
        ...
        $mobile = trim($request->get('mobile', '')); // 身份证号
        $state = trim($request->get('state', ''));  // 状态
        $result = UserLimuModel::getLimuAuthList($limit, $offset, $userName, $mobile, $state, UserLimuModel::TYPE_HOUSE_FUND);
     ...
     * @return string
     */
    public function actionSocialSecurityAuth()
    {
        $data = [];
        $request = Yii::$app->request;
        $offset = $request->get('offset', 0);
        ...
        $mobile = trim($request->get('mobile', '')); // 身份证号
        $state = trim($request->get('state', ''));  // 状态
        $result = UserLimuModel::getLimuAuthList($limit, $offset, $userName, $mobile, $state, UserLimuModel::TYPE_SOCIAL_SECURITY);
     ...
     * @return string
     */
    public function actionCreditAuth()
    {
        $data = [];
        $request = Yii::$app->request;
        $offset = $request->get('offset', 0);
        ...
        $mobile = trim($request->get('mobile', '')); // 身份证号
        $state = trim($request->get('state', ''));  // 状态
        $result = UserLimuModel::getLimuAuthList($limit, $offset, $userName, $mobile, $state, UserLimuModel::TYPE_CREDIT);
    ...
    /**
     * 删除京东、淘宝、学历、社保、 公积金、央行征信、信用卡账单、网银流水认证记录
     * @return string
     */
    ...
    public function actionIncreaseQuota()
    {
        ...
        foreach ($results['result'] as $row) {
            $data[] = [
                ...
                'is_socialsecurity_auth' => $row->is_socialsecurity_auth,// 社保认证
                'is_housefund_auth' => $row->is_housefund_auth, // 公积金认证
                'is_credit_auth' => $row->is_credit_auth, // 央行征信
    ...
    public function actionArea()
    {
        ...
        $area = LimuAreaModel::getAreaByAreaCode($code);
        if (!$area) {
            return Json::encode(['status' => self::STATUS_FAILURE, 'error_message' => '参数错误']);
        }
        return Json::encode([
            'status' => self::STATUS_SUCCESS,
            'results' => $area->area_name ?? ''
        ]);
    }
}