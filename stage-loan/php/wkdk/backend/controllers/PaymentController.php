...
class PaymentController extends BackendController
{
    ...
    public function actionLianlianPayment()
    {
        ...
        if ($loan->type == LoanModel::TYPE_CONSUMPTION && (!isset($loan->shop->actual_controller_mobile) || !isset($loan->shop->actual_controller_name) || !isset($loan->shop->bank_no) || $loan->shop->actual_controller_name == '' || $loan->shop->bank_no == '' || $loan->shop->actual_controller_mobile == '')) {
            return Json::encode([ 'status' => self::STATUS_FAILURE, 'error_message' => '消费分期借款商户信息有误，联系商户完善信息后继续' ]);
        }
        ...
        if ($loan->type == LoanModel::TYPE_CASH) {
            $params = $this->buildCashParam($paylog, $loan, $isReuseFlag); // 构建现金分期放款参数，放款给用户
        } else {
            $params = $this->buildConsumeParam($paylog, $loan, $isReuseFlag); // 构建消费分期放款参数，放款给商户
        }
        ...
        if (!$isReuseFlag) { // 订单不存在 或 不可重复利用时
            // 将订单请求保存至数据库
            $payLogParams['no_order'] = $params['no_order'];
            $payLogParams['info_order'] = $params['info_order'];
        ...
        if ($respose && ($result['ret_code']) == LianlianpayApi::REQUEST_SUCCESS) { // 有返回值，且返回code为0000
            ...
            LoanService::generateGrantingLog($loanId);   // 添加放款日志
    ...
    /**
     * 构建现金分期放款参数
     * @param object $paylog 支付记录对象
     * @param object $loan 借款记录对象
     * @param boolean $isReuseFlag 是否复用支付单号
     * @return array 返回构建完成的放款参数
     */
    private function buildCashParam($paylog, $loan, $isReuseFlag) {
     ...
     * @return array 返回构建完成的放款参数
     */
    private function buildConsumeParam($paylog, $loan, $isReuseFlag)
    {
        $params = $this->lianlianpayInit();
        $orderNo = $isReuseFlag ? $paylog->no_order : LoanService::buildOrderCode($loan->user_id); // 商户系统唯一标识该付款的流水号
        $params['no_order'] = $orderNo; // 商户系统唯一标识该付款的流水号
        ...
        $params['card_no'] = $loan->shop->bank_no; // 收款银行帐号 商户银行账号
        $params['acct_name'] = $loan->shop->actual_controller_name; // 收款人姓名 商户实际控制人姓名
        $params['bank_name'] = $loan->shop->bank_name ?? ''; // 收款银行名称 商户收款银行名称

        $params['info_order'] = LianlianpayApi::APP_CONSUME_ORDER_DES; // 订单描述（付款用途）
        $params['flag_card'] = LianlianpayApi::FLAG_CARD_TO_PRIVATE; // 对公对私标识 0-对私 1-对公
        $params['notify_url'] = Yii::$app->params['llpay_payment_notify'];; // 服务器异步通知地址
        $params['memo'] = LianlianpayApi::PATMENT_MEMO; // 收款备注，传递至银行
        return $params;
