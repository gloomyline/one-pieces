...
class LimuController extends FrontendController
{
    ...
    // 公积金登录元素查询
    public function actionLimuHouseFundLoginElements()
    {
        $request = Yii::$app->request;
        $areaCode = $request->post('area_code'. ''); // 公积金地区代码
        if ($areaCode == '') {
            return Json::encode([
                'status' => self::STATUS_FAILURE,
                'error_message' => '公积金地区代码不能为空'
            ]);
        }
        $result = LimuService::queryHouseFundLoginElements($areaCode);
        if ($result) {
            // 成功
            if ($result->code != LimuService::API_SUCCESS_CODE) {
                return Json::encode([
    ...
    // 公积金查询
    public function actionLimuHouseFund()
    {
        ini_set('max_execution_time','600');
        ...
        $username = $request->post('username', '');
        $password = $request->post('password', '');
        $area = $request->post('area', ''); // 地区
        if (empty($username) || empty($password) || $area == '') {
            return Json::encode(['status' => self::STATUS_FAILURE, 'error_message' => '参数错误']);
        }

        $user = Yii::$app->user->identity;
        // 判断当天是否有认证成功的记录
        $latestCredit = UserLimuModel::getLatestCredit($user->mobile, UserLimuModel::TYPE_HOUSE_FUND);
        if ($latestCredit && $latestCredit->state == UserLimuModel::STATE_PASS) {
            $updatedDay = date('Y-m-d', strtotime($latestCredit->updated_at));
            $today = date('Y-m-d');
            if ($today == $updatedDay) {
        ...
        $result = LimuService::queryHouseFund($user->mobile, $username, $password, $area);
        if ($result) {
            // 受理成功
            if ($result->code == LimuService::API_ACCEPT_SUCCESS_CODE) {
                UserLimuModel::addUserLimu($user->id, $user->mobile, UserLimuModel::STATE_BUSY, $result->token, UserLimuModel::TYPE_HOUSE_FUND);  // 添加数据至数据库
                $count = 200;
                $sleepCount = 3;
                do {
                    $result = LimuService::commonStatusGet($result->token, LimuService::API_BIZ_HOUSE_FUND); // 查询状态
                    if (LimuService::API_SUCCESS_CODE == $result->code) { // 成功，无需输入验证码
                        Yii::info('立木征信公积金查询成功，退出轮询:token->' . $result->token, 'limu');
                        return Json::encode([
                            'status' => self::STATUS_SUCCESS,
                            'error_message' => '认证成功',
                            "results" => [['token' => $result->token]]
                        ...
                        Yii::info('立木征信公积金查询登入成功，退出轮询:token->' . $result->token, 'limu');
                        return Json::encode([
                            'status' => self::STATUS_SUCCESS,
                            'error_message' => $result->code,
                            "results" => [['token' => $result->token]]
    ...
    // 社保登录元素查询
    public function actionLimuSocialSecurityLoginElements()
    {
        $request = Yii::$app->request;
        $areaCode = $request->post('area_code'. ''); // 公积金地区代码
        if ($areaCode == '') {
            return Json::encode([
                'status' => self::STATUS_FAILURE,
                'error_message' => '社保地区代码不能为空'
            ]);
        }
        $result = LimuService::querySocialSecurityLoginElements($areaCode);
        if ($result) {
            // 成功
            if ($result->code != LimuService::API_SUCCESS_CODE) {
                return Json::encode([
    ...
    // 社保查询
    public function actionLimuSocialSecurity()
    {
        ini_set('max_execution_time','600');
        ...
        $user = Yii::$app->user->identity;
        // 判断当天是否有认证成功的记录
        $latestCredit = UserLimuModel::getLatestCredit($user->mobile, UserLimuModel::TYPE_SOCIAL_SECURITY);
        if ($latestCredit && $latestCredit->state == UserLimuModel::STATE_PASS) {
            $updatedDay = date('Y-m-d', strtotime($latestCredit->updated_at));
            $today = date('Y-m-d');
            if ($today == $updatedDay) {
        ...
        $result = LimuService::querySocialSecurity($user->mobile, $username, $password, $area);
        if ($result) {
            // 受理成功
            if ($result->code == LimuService::API_ACCEPT_SUCCESS_CODE) {
                UserLimuModel::addUserLimu($user->id, $user->mobile, UserLimuModel::STATE_BUSY, $result->token, UserLimuModel::TYPE_SOCIAL_SECURITY);  // 添加数据至数据库
                $count = 200;
                $sleepCount = 3;
                do {
                    $result = LimuService::commonStatusGet($result->token, LimuService::API_BIZ_SOCIAL_SECURITY); // 查询状态
                    if (LimuService::API_SUCCESS_CODE == $result->code) { // 成功，无需输入验证码
                        Yii::info('立木征信社保查询成功，退出轮询:token->' . $result->token, 'limu');
                        return Json::encode([
                            'status' => self::STATUS_SUCCESS,
                            'error_message' => '认证成功',
                            "results" => [['token' => $result->token]]
                        ...
                        Yii::info('立木征信社保查询登入成功，退出轮询:token->' . $result->token, 'limu');
                        return Json::encode([
                            'status' => self::STATUS_SUCCESS,
                            'error_message' => $result->code,
                            "results" => [['token' => $result->token]]
    ...
    // 央行征信查询
    public function actionLimuCreditAuth()
    {
        ini_set('max_execution_time','600');
        ...
        $password = $request->post('password', ''); // 密码
        $middleAuthCode = $request->post('middle_auth_code', ''); // 央行征信身份验证码
        if (empty($username) || empty($password)) {
            return Json::encode(['status' => self::STATUS_FAILURE, 'error_message' => '参数错误']);
        }
        ...
        $user = Yii::$app->user->identity;
        // 判断当天是否有认证成功的记录
        $latestCredit = UserLimuModel::getLatestCredit($user->mobile, UserLimuModel::TYPE_CREDIT);
        if ($latestCredit && $latestCredit->state == UserLimuModel::STATE_PASS) {
            $updatedDay = date('Y-m-d', strtotime($latestCredit->updated_at));
            $today = date('Y-m-d');
            if ($today == $updatedDay) {
        ...
        $result = LimuService::queryCredit($user->mobile, $username, $password, $middleAuthCode);
        if ($result) {
            // 受理成功
            if ($result->code == LimuService::API_ACCEPT_SUCCESS_CODE) {
                UserLimuModel::addUserLimu($user->id, $user->mobile, UserLimuModel::STATE_BUSY, $result->token, UserLimuModel::TYPE_CREDIT);  // 添加数据至数据库
                $count = 200;
                $sleepCount = 3;
                do {
                    $result = LimuService::commonStatusGet($result->token, LimuService::API_BIZ_CREDIT); // 查询状态
                    if (LimuService::API_WAIT_INPUT_CODE == $result->code) { // 等待输入验证码
                        Yii::info('立木征信央行征信查询提示输入验证码:token->' . $result->token, 'limu');
                        return Json::encode([
                            'status' => self::STATUS_SUCCESS,
                            'error_message' => $result->code,
                            "results" => [['token' => $result->token]]
                        ...
                        Yii::info('立木征信央行征信查询成功，退出轮询:token->' . $result->token, 'limu');
                        return Json::encode([
                            'status' => self::STATUS_SUCCESS,
                            'error_message' => '认证成功',
                            "results" => [['token' => $result->token]]
                        ...
                        Yii::info('立木征信央行征信查询登入成功，退出轮询:token->' . $result->token, 'limu');
                        return Json::encode([
                            'status' => self::STATUS_SUCCESS,
                            'error_message' => $result->code,
                            "results" => [['token' => $result->token]]
