...
class CommonService extends Component
{
     ...
     * @param integer $budgetPlanId 借款分期计划id 催收时逾期群发通知需赋值
     * @return array
     */
    public function sendSmsCode($sign, $msgTemplate, $mobile, $param, $type, $budgetPlanId = null)
    {
     ...
     * @param string $type 短信的类型
     * @param integer $budgetPlanId 分期计划id 催收时逾期群发通知需赋值
     * @return array
     */
    public function sendPhoneCode($mobile, $msgTemplate, $param, $type, $budgetPlanId = null)
    {
        if (empty($mobile) or ! preg_match("/^1[0-9]{10}$/", $mobile)) {
            return ['code' => 2000, 'message' => '请输入可用的手机号码'];
        }
