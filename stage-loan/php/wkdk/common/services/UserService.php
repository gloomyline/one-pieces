...
class UserService
{
    ...
    public static function setProfileAuth($userId)
    {
        ...
        if (isset($userBasic->is_work_auth) && $userBasic->is_work_auth == UserBasicModel::WORK_AUTH_CERTIFIED && $userBasic->is_relation_auth == UserBasicModel::RELATION_AUTH_CERTIFIED && $userBasic->live_area) {
            $user = User::findOne(['id' => $userId]);
            $user->is_profile_auth = UserModel::HAS_PROFILE_AUTH; // 更新状态
    ...
    public static function setUserLimuAuth($token, $content, $platformType = UserLimuModel::TYPE_JD)
    {
        ...
        if ($platformType == UserLimuModel::TYPE_JD) {
            $user->is_jd_auth = UserModel::HAS_JD_AUTH; // 京东已认证
        } else if($platformType == UserLimuModel::TYPE_TAOBAO) {
            $user->is_taobao_auth = UserModel::HAS_TAOBAO_AUTH; // 淘宝已认证
        } else if($platformType == UserLimuModel::TYPE_EDUCATION) {
            $user->is_edu_auth = UserModel::HAS_EDU_AUTH; // 学历已认证
            $user->education = $content->educationInfo->arrangement  ?? ''; // 学历信息->层次
        } else if ($platformType == UserLimuModel::TYPE_BILL) {
            $user->is_bill_auth = UserModel::HAS_BILL_AUTH; // 信用卡账单已认证
        } else if ($platformType == UserLimuModel::TYPE_EBANK) {
            $user->is_ebank_auth = UserModel::HAS_EBANK_AUTH; // 网银流水已认证
        } else if ($platformType == UserLimuModel::TYPE_HOUSE_FUND) {
            $user->is_housefund_auth = UserModel::HAS_HOUSE_FUND_AUTH; // 公积金已认证
        } else if ($platformType == UserLimuModel::TYPE_HOUSE_FUND) {
            $user->is_socialsecurity_auth = UserModel::HAS_SOCIAL_SECURITY_AUTH;// 社保已认证
        } else if ($platformType == UserLimuModel::TYPE_CREDIT) {
            $user->is_credit_auth = UserModel::HAS_CREDIT_AUTH; // 央行征信已认证
        }
        $user->updated_at= date('Y-m-d H:i:s'); // 更新时间
        if ($user->save()) { // 保存成功
            return true;
