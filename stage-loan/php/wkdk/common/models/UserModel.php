...
class UserModel extends CommonModel
{
    ...
    const HAS_PICTURE_AUTH = 1; // 亲签照已认证
    const HAS_CREDIT_AUTH = 1; // 央行征信已认证
    const HAS_HOUSE_FUND_AUTH = 1; // 公积金已认证
    const HAS_SOCIAL_SECURITY_AUTH = 1; // 社保已认证
    ...
    const HAS_NO_PICTURE_AUTH = 0; // 亲签照未认证
    const HAS_NO_CREDIT_AUTH = 0; // 央行征信未认证
    const HAS_NO_HOUSE_FUND_AUTH = 0; // 公积金未认证
    const HAS_NO_SOCIAL_SECURITY_AUTH = 0; // 社保未认证
     ...
     */
    public static function getUserList($offset, $limit, $realName, $mobile, $state, $beginAt, $endAt)
    {
        $result = $data = [];
        $userModel = User::find();
        if ($state) {
     ...
     * @param integer $userId 用户ID
     * @param integer $quota 金额
     * @param boolean $needUpdateRepayCount 是否需要更新成功还款次数
     * @return boolean 是否更新成功 true-是 false-否
     */
    public static function thawUserQuota($userId, $quota, $needUpdateRepayCount = false)
    {
        $user = User::findOne(['id' => $userId]);
        $user->fronzen_quota -= $quota; // 冻结额度
        $user->available_quota += $quota; // 可用额度 = 原来可用额度 + 还款额度
     ...
     * @param integer $userId 用户id
     */
    public static function setUserPictureAuth($userId)
    {
        $user = User::findOne(['id' => $userId]);
        if (empty($user)) {
            return;
        }
        $user->is_picture_auth = UserModel::HAS_PICTURE_AUTH; // 亲签照认证通过
        $user->save();
    }
    /**
     * 更新用户手机认证状态
