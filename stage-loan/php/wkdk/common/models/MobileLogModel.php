...
class MobileLogModel extends CommonModel
{
    ...
    const TYPE_SHOP_CONFIRM = 'shop_confirm'; // 商家确认提醒
     ...
     */
    public static function addList($mobile,$mess,$code,$content,$type,$budgetPlanId = null)
    {
        ...
        if (!empty($budgetPlanId)) {
            $logModel->budget_plan_id = $budgetPlanId;
        }
    ...
    /**
     * 查找是否存在该借款订单分期计划的最新逾期群发通知记录
     * @param integer $budgetPlanId 借款订单分期id
     * @param string $mobile 接收短信的手机号码
     * @return array|null|\yii\db\ActiveRecord
     */
    public static function findOverdueMassByBudgetPlanIdAndMobile($budgetPlanId, $mobile)
    {
        return MobileLog::find()->where(['mobile' => $mobile, 'budget_plan_id' => $budgetPlanId, 'type' => self::TYPE_OVERDUE_MASS])->orderBy(['id' => SORT_DESC])->one();
    }
}