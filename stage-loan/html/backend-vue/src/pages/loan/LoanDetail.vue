    ...
    <div style="background-color: #f2f2f2;padding: 20px;overflow: auto;">
      ...
      <el-row class='backStyle' style=''>
          ...
          <el-row>
              ...
              <el-col :span="6">
                  <div class="basicStyle" style=''>
                      <el-button type="text" size="small" @click.native="handleJumpCredit(borrower.mobile)" v-if="auth.is_credit_auth === 1">查看报告</el-button>
              ...
              <el-col :span="6">
                  <div class="basicStyle" style=''>
                      <el-button type="text" size="small" @click.native="handelJumpJd(borrower.mobile)" v-if="auth.is_jd_auth === 1">查看报告</el-button>
      ...
      <el-row class='backStyle' style=''>
        ...
        <el-row>
            ...
            <el-col :span="6">
                <div class="basicStyle" style=''>
                    <span v-if="reviews.review_result === 'review_success' || reviews.review_result === 'granting' || reviews.review_result === 'repaying' || reviews.review_result === 'finished' || reviews.review_result === 'overdue'" style="color: limegreen ">{{ reviews.review_result | reviewResultFilter }}</span>
                    <span class="unfilled-red" v-if="reviews.review_result === 'review_failure'">{{ reviews.review_result | reviewResultFilter }}</span>
                    <span v-if="reviews.review_result === ''">{{ reviews.review_result | reviewResultFilter }}</span>
                </div>
            </el-col>
        </el-row>
      </el-row>
        ...
        this.$http.post(`${apiBase}update-anti-fraud`, params)
          ...
          .then((json) => {
            if (json.status === 'SUCCESS') {
              this.$http.get(`${apiBase}anti-fraud/${this.borrower.user_id}`)
                  .then(getJsonAndCheckSuccess)
                  .then((results) => {
                    this.fraud = results.results.fraud;
                  }).catch(reportErrorMessage(this));
            }
          }).catch(reportErrorMessage(this));
      } else {
