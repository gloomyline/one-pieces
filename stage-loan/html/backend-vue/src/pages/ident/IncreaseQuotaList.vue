<template>
  <div>
    ...
    <el-table :data="tableData" stripe border style="width: 100%;">
      ...
      <el-table-column property="is_socialsecurity_auth" label="社保认证" align="center" min-width="100">
        <template slot-scope="scope">
          <el-button type="success" size="small" @click.native="handleJumpSocialSecurity(scope.row.mobile)" v-if="scope.row.is_socialsecurity_auth === 1">查看报告</el-button>
          <span class="unfilled-red" v-else="scope.row.is_socialsecurity_auth === 0">未填写</span>
        </template>
      </el-table-column>
      <el-table-column property="is_housefund_auth" label="公积金认证" align="center" min-width="100">
        <template slot-scope="scope">
          <el-button type="success" size="small" @click.native="handleJumpHouseFund(scope.row.mobile)" v-if="scope.row.is_housefund_auth === 1">查看报告</el-button>
          <span class="unfilled-red" v-else="scope.row.is_housefund_auth === 0">未填写</span>
        </template>
      </el-table-column>
      <el-table-column property="is_credit_auth" label="央行征信认证" align="center" min-width="120">
        <template slot-scope="scope">
          <el-button type="success" size="small" @click.native="handleJumpCredit(scope.row.mobile)" v-if="scope.row.is_credit_auth === 1">查看报告</el-button>
          <span class="unfilled-red" v-else="scope.row.is_credit_auth === 0">未填写</span>
        </template>
      </el-table-column>
