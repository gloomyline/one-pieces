...
export default {
  ...
  methods: {
    ...
    confirm () { // goto page bankcard/add
      this.$router.push('/authentication/bankcard/add')
    },
    detail (bankcardId) { // look over the detail of bankcard which index is preferenced
      this.$router.push(`/authentication/bankcard/detail/${bankcardId}`)
    }
  },
  // filters: {
  //   bankCode2Id (bankCode) {
  //     let card = bankData.find((el) => {
  //       return el.code === bankCode
  //     })
