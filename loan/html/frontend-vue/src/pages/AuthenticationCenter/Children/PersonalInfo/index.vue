<template>
  <div class="page-personal-info">
    ...
    <group class="input-area" gutter="0">
      ...
      <cell class="job" :class="{actived: userInfo.is_work_auth === 1}" title="工作信息" :value="userInfo.is_work_auth | statusFormatter" is-link link="/authentication/personalInfo/job"></cell>
      <cell class="relationship" :class="{actived: userInfo.is_relation_auth === 1}" title="人际关系" :value="userInfo.is_relation_auth | statusFormatter" is-link link="/authentication/personalInfo/relationship"></cell>
    </group>
...
export default {
  ...
  watch: {
    ...
    // 'isUserInfoConfirmed' () {
    //   if (this.isUserInfoConfirmed) { // go back after saving informations of user to the server successfully
    //     console.log(11)
    //     this.$router.go(-1)
    //   }
    // }
