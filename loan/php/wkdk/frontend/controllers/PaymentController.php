...
class PaymentController extends FrontendController
{
    ...
    public function actionLianlianpaySign()
    {
        ...
        $params['risk_item'] = '{"frms_ware_category":"2010","user_info_mercht_userno":"'. $userId . '","user_info_bind_phone":"' . $userMobile . '","user_info_dt_register":"' . date('YmdHis',strtotime($userRegistTime)) . '","user_info_identify_type":"1","user_info_identify_state":"1","user_info_full_name":"' . $userRealName . '","user_info_id_no":"' . $userIdentityNo . '"}';
    ...
    public function actionLianlianpaySignReturn()
    {
        ...
        if ($status != '0000') {
            ...
            return $this->redirect(["/m/#/authentication/bankcard/addResults?status={$status}&result={$result}"]);
        }
        ...
        return $this->redirect(['/m/#/authentication/bankcard/addResults?status=0000']);
    ...
    public function actionLianlianAuthPay()
    {
        ...
        $money_order = $repayDetail['repaymentAmount']; // 交易金额

        $params = $this->lianlianpayInit(); // 最基本的系统参数，如商户编号、版本号、支付签名方式等
        $params['user_id'] = (string)$userId; // 商户用户唯一编号
        ...
        $params['risk_item'] = '{"frms_ware_category":"2010","user_info_mercht_userno":"'. $userId . '","user_info_bind_phone":"' . $userMobile . '","user_info_dt_register":"' . date('YmdHis',strtotime($userRegistTime)) . '","user_info_identify_type":"1","user_info_identify_state":"1","user_info_full_name":"' . $userRealName . '","user_info_id_no":"' . $userIdentityNo . '"}';// 风险控制参数
    ...
    public function actionLianlianAuthPayUrlReturn()
    {
        ...
        if ($result) { // 支付成功
            ...
            if (!$verifyResult) {
                ...
                // 其余失败或异常情况 跳转至查单页面
                return $this->redirect(["/m/#/home/refund/fail"]);// 跳转至提示页面
            }
            ...
            if ($payLog->state != PayLogModel::STATE_SUCCESS) { // 订单未未成功，需处理数据；订单已成功，不做处理
                if (strtolower($data['result_pay']) == PayLogModel::STATE_PROCESSING) { // 支付订单正在处理中
                    // 添加日志
                    $repayDeatil = LoanService::caculateRepaymentAmountDetail($payLog->loan_id); // 计算 用户还款本金、还款金额、违约金
                    LoanService::generatePayingLog($payLog->loan_id, $repayDeatil['quota'], $repayDeatil['repaymentAmount'], $repayDeatil['overdueAmount'], $repayDeatil['interest']);
                }
            } // if 是否更改数据
            return $this->redirect(["/m/#/home/refund/success"]);// 跳转至提示支付成功页面
        }
