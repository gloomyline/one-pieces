        ...
        if ($result) { // 成功状态
            ...
            if ($payLog->state != PayLogModel::STATE_SUCCESS) {
                try {
                    ...
                    LoanService::updateCorrelationAfterRepaySuccess($loan->id, $loan->quota, $repayedAmount, $payLog->user_id, $loan->state);

                    $im = Yii::$app->companyim;
                    $im->sendTextToChatGroup($im::SENDER_XIAOBAI, $im::COMPANY_CHAT_ID, sprintf('用户还款成功！用户ID(%s),金额(%s)！', $payLog->user_id, $repayedAmount));
