...
class UrgeModel extends  CommonModel
{
     ...
     * @return static
     */
    public static function getUrgeByLoanId($loanId)
    {
        return Urge::findOne(['loan_id' => $loanId]);
    }

    /**
     * 根据催收记录id获取催收记录
     * @param integer $id  催收记录id
     * @return static 返回一条催收记录了
     */
    ...
    public static function getUrgeList($offset, $limit, $realName, $mobile, $adminId, $state, $beginAt, $endAt)
    {
        ...
        if ($beginAt != '') {
            ...
            $urge->andWhere(['>=', 'loan.planned_repayment_at', $beginAt]); // 预计还款 起始时间
        }
        if ($endAt != '') {
            $endAt = date('Y-m-d H:i:s', (int)($endAt) + 3600 * 24 - 1); // 时间戳转字符串, 截止时间设置为当前日期的 23:59:59，如 2017-09-20 23:59:59
            $urge->andWhere(['<=', 'loan.planned_repayment_at', $endAt]); // 预计还款 截止时间
        }
        return [
            'count' => $urge->count(),
            'result' =>  $urge->offset($offset)->limit($limit)->all()
        ];
    }
     ...
     */
    public static function urgeFinished($loanId)
    {
        $model = Urge::findOne(['loan_id' => $loanId]);
        if ($model) {
            $model->state = self::STATE_FINISHED;
            if (!$model->save()) {
                return false;
            }
        }
